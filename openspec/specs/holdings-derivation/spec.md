...
Position 的成本 SHALL 依 planning doc §4 加權平均法計算：`totalCost = Σ(amounts[original_currency].total) + Σfee + Σtax`（**手續費與交易稅皆計入成本基礎**）、`averageCost = totalCost / quantity`。所有運算 SHALL 用 `Money`（decimal.js），輸出 10 位小數 string，禁 native float。§4 worked example SHALL 直接作為測試 fixture。
...
- **WHEN** 同一 `(market, symbol)` 出現不同 `original_currency` 的交易（理論上不應發生）
- **THEN** 函式 SHALL 以明確錯誤拒絕（fail loud），不得靜默混算不同幣別（CurrencyMismatchError 或等價防護）

### Requirement: 持倉清單檢視（HoldingsOverview）

持倉 Tab SHALL 顯示由 `deriveHoldings` 推導的持倉清單：依 `market` 分組（TW / US），每組顯示該市場**原幣別總成本小計**；每列顯示 `symbol`、總股數、加權均價與總成本。**不顯示市值 / 損益**（需現價，Sprint 5）；**不跨幣別加總**（需匯率，Sprint 4）。金額顯示 SHALL 用 `Money.toDisplayString()`（2 位小數），儲存與計算仍 10 位。
...
點擊持倉清單任一列 SHALL 進入該股 AssetDetail，顯示：持倉摘要（總股數、加權均價、總成本、原幣別）+ **對帳 timeline**——該 `(market, symbol)` 的全部交易依 `transaction_date` 由舊到新排序（買進累積的時間順序），每筆顯示日期、股數、單價、手續費。本畫面**無現價、無多幣別切換**（Sprint 4/5）。
...
持倉推導 SHALL 完全由既有 `transactionsStore` 的資料計算（`useMemo`），**不得**新增 Firestore 讀取、不改 rules、不改 schema。`deriveHoldings` SHALL 為不依賴 Firestore / React 的純函式，先寫測試再實作（TDD），納入 `packages/shared` ≥90% coverage gate。
...
#### Scenario: 不新增 Firestore 監聽

- **WHEN** 使用者瀏覽持倉清單與 AssetDetail
- **THEN** App 對 Firestore 的監聽數量與 Change 1 結束時相同（僅 accounts + transactions 兩個 onSnapshot）
