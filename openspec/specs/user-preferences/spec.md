...
使用者個人資料（顯示名稱）與顯示偏好（顯示幣別 `preferred_display_currency`）的編輯與持久化：從 `users/{uid}` 載入現值、寫回前以 `packages/shared` 純函式驗證、回饋成功/失敗。顯示幣別偏好由 app-wide store 持有並跨畫面套用（持倉總覽、分析頁），其切換控制設於持倉總覽頁。
