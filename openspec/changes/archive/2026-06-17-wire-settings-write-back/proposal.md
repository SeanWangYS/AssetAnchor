...
- **顯示幣別切換移至持倉頁（owner 設計決策 2026-06-17）**：TWD/USD 切換從設定移到**持倉總覽頁「資產走勢」走勢圖之上**，此切換即偏好控制（切換→持久化 `preferred_display_currency`→帶動全 app）。**原「顯示偏好」設定子頁移除**（dark-mode 先前已移除、幣別又移出 → 整頁清空），設定「偏好」組剩「個人資料」。
- **跨畫面套用 `preferred_display_currency`**：新增 app-wide **`services/preferences`** store（自帶 Firestore 持久化；住 services 對照 exchange-rates、守依賴方向 #4），登入時由 `users/{uid}` 灌入、登出 reset。**持倉總覽所有金額**讀偏好（NT$↔US$、label/小數位隨之切）：真實「總成本」合計 + Hero 總資產 / 總未實現 / 今日 / 本月已實現損益等 demo 摘要；**百分比**（總報酬率、今日 %）不換算。**分析頁 TWD/USD 切換預設**讀偏好（使用者仍可頁內覆寫）。個股詳情頁按 spec 維持「預設原幣別」。
- **服務層**：`apps/mobile/src/features/auth/userDoc.ts` 的 `updateUserProfile`（display_name 雙寫 Firestore + Auth）；顯示幣別持久化改由 `services/preferences` store 直接以 modular API（v24）`updateDoc` 寫入（不經 userDoc，避免 holdings→auth feature import）。
...
- `analysis`: 「分析頁 TWD/USD 全頁切換」的**預設值**由固定 `TWD` 改為**讀顯示幣別偏好**（缺值 fallback TWD）；切換行為不變、使用者仍可頁內覆寫。
- `currency-display`: 「持倉總覽跨幣別總成本合計」**推翻**原「本畫面不提供切換鈕」之約束——HoldingsOverview 改為於走勢圖上提供 TWD/USD 切換鈕＝偏好控制，本頁所有金額隨偏好切（百分比不換）。

## Impact

- **程式碼**：`features/settings/screens/ProfileScreen.tsx`（display_name 寫回）；**刪除 `DisplayPrefsScreen.tsx` + `SettingsStack`/`SettingsScreen`/nav types 的「顯示偏好」入口**；`features/auth/userDoc.ts`（`updateUserProfile`；移除 `updateDisplayCurrency`）；`packages/shared/src/preferences/`（驗證純函式 + 型別 + 測試）；**新增 `apps/mobile/src/services/preferences/`（app-wide store + 持久化）**；`App.tsx`（登入 hydrate / 登出 reset）；`features/holdings/screens/HoldingsOverviewScreen.tsx`（**新增幣別切換鈕** + hero/bento/總成本讀偏好）+ `holdingsDemo.ts`（移除 dead `DISPLAY_CURRENCY`）；`features/analysis/screens/AnalysisOverviewScreen.tsx`（切換預設讀偏好）。
