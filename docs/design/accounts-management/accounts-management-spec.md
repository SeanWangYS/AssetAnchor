...
| A2 | 列表 row：色標 monogram＋帳戶名／類型·市場；右側＝該帳戶持股**市值**（原幣別，真實報價；缺報價降級「更新中」/最後已知） | 不放現金/檔數，保持單行。⚠️ 2026-07-10 釐清：市值＝真實報價（非成本代理），對齊報價上線後語意（change `account-detail-market-value`） |
| A3 | 已停用帳戶收在列表底部，灰階降明度、市值顯示 — | 對應 `is_active` 軟刪除 |
| A4 | 新增帳戶入口＝**FAB**（同交易頁慣例，無 header ＋） | |
| A5 | 詳情 hero＝**帳戶市值**（持股市值＋現金，基礎幣別）＋拆分小字「持股市值 X · 現金 Y」＋並列「投入成本 X · 未實現損益 ±Y（±Z%）」（`Pnl` 紅綠 ▲▼）。持股列右側＝市值＋報酬%（原幣別，均價留 subtitle）。報價缺失/過期複用 live-quotes 降級（更新中／查無代號／最後已知＋重試）；多幣別換算進基礎幣別；持股列表維持無色點（A1） | ⚠️ 2026-07-10 增修（owner 核准，B 案）：由「帳戶總值＝成本代理」改真實市值＋成本/未實現並列，對齊原型市值語意＋報價上線。change `account-detail-market-value` |
