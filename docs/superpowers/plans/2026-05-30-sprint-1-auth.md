...
**Files:**
- Create: `.tool-versions`（asdf，本機用）+ `.nvmrc`（nvm 用戶 / 部分 CI 用）
...
- [ ] **Step 1：切換本機到 Node 22**

> **本機用的是 asdf**（已查證；不是 nvm，asdf global 釘 nodejs 24.15.0）。asdf/nvm/pyenv 都是「在一台機器管理多版本 runtime」的工具。

**asdf（本機）**：
```bash
asdf install nodejs 22.22.3
# 下一步建立 repo 的 .tool-versions 後，asdf 進到本 repo 會自動切 22
cd /Users/sean.wang/Documents/Sean/project/AssetAnchor && node -v   # 預期 v22.x
```
**（若改用 nvm 的環境）**：`nvm install 22 && nvm use 22`（讀 `.nvmrc`）。

- [ ] **Step 2：釘專案 Node 版本（兩個 manager 各一檔）**

`/Users/sean.wang/Documents/Sean/project/AssetAnchor/.tool-versions`（asdf 讀這個）：
```
nodejs 22.22.3
```
`/Users/sean.wang/Documents/Sean/project/AssetAnchor/.nvmrc`（nvm 讀這個）：
```
22
```
> asdf 讀 `.tool-versions`、nvm 讀 `.nvmrc`、CI 用 `actions/setup-node` 的 `NODE_VERSION=22`（已是）。三者都指向 Node 22。
...
```bash
git add .tool-versions .nvmrc apps/functions/package.json
git commit -m "chore(infra): unify Node 22 across dev/CI/functions (asdf .tool-versions + .nvmrc + engines)"
```
...
2. iPhone 與 Mac 在同一區網：同一 Wi-Fi，**或筆電連手機熱點**（熱點下兩者同子網，LAN 模式可通）。macOS 第一次會問是否允許 node 連入 → 按允許。
3. `pnpm start` 後，用 iPhone 相機掃描終端機 QR code（或 Expo Go 內掃描）。
   - **連不上 → tunnel 模式**：`pnpm start --tunnel`（走 Expo 中繼、跨任何網路皆可，較慢；首次裝 `@expo/ngrok`）。
