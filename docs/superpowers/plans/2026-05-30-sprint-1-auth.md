...
## Task 0：Node 版本 reconcile（消除 engines 不一致）

**Files:**
- Create: `.nvmrc`
- Modify: `package.json`（root，`engines`）
- Modify: `apps/functions/package.json`（`engines`）

> 背景：環境是 node v24，root `engines` 寫 `>=22`，`apps/functions` 寫 `node: "20"`（Firebase Functions runtime）。`pnpm install` 會對 functions 噴 engine warning。目標：定一個 dev canonical 版本、讓 warning 消失、且不破壞 functions 的部署 runtime 宣告。

- [ ] **Step 1：確認 Expo SDK 54 與 Firebase Functions 的 Node 需求**

Expo SDK 54 支援 Node 20 / 22（LTS）。Firebase Functions 第二代支援 node 20/22。**決策：dev canonical = Node 22 LTS**（同時滿足 Expo 與 Functions，且 root `engines: >=22` 不變）。

- [ ] **Step 2：建立 `.nvmrc` 釘 dev Node 版本**
...
> 若你目前在 node 24：`nvm install 22 && nvm use`（沒有 nvm 就先裝 nvm，或忽略——24 也能跑，但團隊/CI 對齊到 22 較穩）。

- [ ] **Step 3：把 `apps/functions` 的 engines 對齊到 20（Functions runtime）並加註解說明**

Functions 部署 runtime 仍是 20（Firebase 限制），這是**刻意**與 dev 不同。確認 `apps/functions/package.json` 的 `engines` 寫法不會在 install 時被當成 root 衝突 —— pnpm 的 engine warning 來自「目前 node 版本 > functions 宣告」。把 dev 切到 node 22 後，warning 仍會出現（22 ≠ 20）。**解法：在 functions package.json 用 `engines.node: "20"` 保留部署宣告，並在 root `.npmrc` 關閉 engine 嚴格檢查避免 noise**：

`/Users/sean.wang/Documents/Sean/project/AssetAnchor/.npmrc`（append 一行，保留既有 `node-linker=hoisted`）：
```
node-linker=hoisted
engine-strict=false
```

> `engine-strict=false` 讓 pnpm 對 engines 不一致只警告不擋（本來預設就不擋，但顯式關閉可消 noise）。functions 的 `node: "20"` 保留為「部署目標」語意。

- [ ] **Step 4：驗證 install 無 engine 噪音 + CI 仍綠**
...
```
Expected: install 不再因 engines 中斷/噴顯著 warning；typecheck/lint/test 全綠（packages/shared 76 tests pass）。

- [ ] **Step 5：Commit**

```bash
git add .nvmrc .npmrc package.json apps/functions/package.json
git commit -m "chore(infra): pin dev Node 22 via .nvmrc + quiet engine-strict warnings"
```
