# Feynman Dojo Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 建立 Feynman Dojo 內化訓練系統：一個 `/feynman-dojo` skill＋教材底稿（curriculum）＋進度帳本（ledger），依 `docs/superpowers/specs/2026-07-13-feynman-dojo-design.md` 施工。

**Architecture:** 三個 markdown 檔案構成閉環——SKILL.md 是流程引擎（讀帳本→跑 session 協議→寫回帳本），curriculum 是唯讀教材（七項架構主題的證據＋題庫種子），ledger 是唯一可變狀態（項目狀態、洞佇列、session log）。全部進 git。

**Tech Stack:** Markdown、Claude Code project skill（`.claude/skills/`）、prettier、Conventional Commits（commitlint scope 限 `mobile/functions/shared/infra/docs/firebase`）。

## Global Constraints

- 絕不 push/merge `main`；一律 feature 分支 + PR（global CLAUDE.md 安全規則）。
- 分支基準＝剛 fetch 的最新 `origin/main`（`40c7a47`，GitHub Flow 憲法 #8）。
- 工作區現有 `docs/design/README.md`（modified）與 `docs/design/transaction-import/`（untracked）為 owner 進行中的 transaction-import 設計工作——**絕不 add、絕不 commit、絕不還原**。
- `.gitignore` 第 69 行為 `.claude/`；`.claude/skills/` 內現有 5 個 openspec-* 本地 skill 必須**維持不被追蹤**，例外只放行 `feynman-dojo`。
- 所有新 markdown 需通過 `pnpm exec prettier --check .`（CI lint job 會跑）。
- Commit 訊息走 commitlint；本計畫用 `chore(infra)`（gitignore）與 `docs(docs)`（內容）。
...
- Consumes: `origin/main @ 40c7a47`；untracked 檔 `docs/superpowers/specs/2026-07-13-feynman-dojo-design.md`、`docs/superpowers/plans/2026-07-13-feynman-dojo.md` 會自動跟著工作區走。
- Produces: 分支 `feature/feynman-dojo`，後續所有 task 在此分支上執行。

- [ ] **Step 1: 建分支**

```bash
git fetch origin main && git checkout -b feature/feynman-dojo origin/main
```

Expected: `Switched to a new branch 'feature/feynman-dojo'`。若 checkout 因 `docs/design/README.md` 衝突被拒（"would be overwritten"），**停下回報 owner**，不得 stash/checkout --force。
...
Expected: 仍看到 ` M docs/design/README.md`、`?? docs/design/transaction-import/`、`?? docs/superpowers/`（可能還有 `?? docs/learning/`＝尚未建立）。transaction-import 的變更原封不動。

---

### Task 2: gitignore 精準例外
...
- Produces: `.claude/skills/feynman-dojo/**` 可被 git 追蹤；其餘 `.claude/**` 照舊 ignore。Task 5 依賴此規則。

- [ ] **Step 1: 改寫 ignore 規則**

把第 69 行的單行 `.claude/` 替換為（gitignore 規則：父目錄被整個 exclude 時內部 negation 無效，故必須改成 `/*` 形式逐層放行）：

```gitignore
.claude/*
!.claude/skills/
.claude/skills/*
!.claude/skills/feynman-dojo/
```

- [ ] **Step 2: 驗證放行與維持忽略**

```bash
git check-ignore -v .claude/skills/openspec-explore/SKILL.md; echo "exit=$?"
git check-ignore -v .claude/settings.local.json; echo "exit=$?"
git check-ignore -v .claude/skills/feynman-dojo/SKILL.md; echo "exit=$?"
```

Expected: 前兩條命中 ignore 規則（exit=0）；第三條**無輸出且 exit=1**（不被忽略）。

- [ ] **Step 3: Commit**

```bash
git add .gitignore
git commit -m "chore(infra): gitignore 精準放行 .claude/skills/feynman-dojo（其餘 .claude 照舊忽略）"
```

---

### Task 3: 教材底稿 `docs/learning/feynman-curriculum.md`

**Files:**

- Create: `docs/learning/feynman-curriculum.md`

**Interfaces:**

- Produces: 七個項目的固定 ID `item-1` … `item-7`（ledger 與 SKILL.md 以此 ID 對照）；每項含「核心概念／錨定證據／題庫種子（事實層·因果層·遷移層）／畢業 note 建議切角」四節。

- [ ] **Step 1: 寫入完整教材內容**

完整內容如下（執行時原樣寫入；此檔為唯讀教材，session 中不修改）：
...
- **全專案最漂亮的對比——同一人對兩種資料做相反決定，且都對**：
  ...
  - price_history **落地**（ADR-0010，`price_history/{symbolId}_{year}` per-symbol-per-year 分片）：近不可變、可回填、批量讀 → 不落地＝每次開圖打爆上游（429）。
  - 決策軸不是技術偏好，是**資料性質**。
...
```bash
pnpm exec prettier --write docs/learning/feynman-curriculum.md
```
...
### Task 4: 進度帳本 `docs/learning/feynman-ledger.md`

**Files:**

- Create: `docs/learning/feynman-ledger.md`

**Interfaces:**

- Consumes: curriculum 的 `item-1`…`item-7` ID。
- Produces: SKILL.md 每次 session 讀寫的狀態格式（項目狀態表、洞佇列表、session log）。
...
```bash
pnpm exec prettier --write docs/learning/feynman-ledger.md
```

---

### Task 5: 流程引擎 `.claude/skills/feynman-dojo/SKILL.md`

**Files:**

- Create: `.claude/skills/feynman-dojo/SKILL.md`

**Interfaces:**

- Consumes: curriculum（唯讀）＋ ledger（讀寫）的路徑與格式（Task 3/4 定義）。
- Produces: 使用者輸入 `/feynman-dojo` 時的完整 session 行為。
...
- [ ] **Step 2: 驗證 skill 檔可被追蹤且格式乾淨**

```bash
git check-ignore .claude/skills/feynman-dojo/SKILL.md; echo "exit=$?"
pnpm exec prettier --write .claude/skills/feynman-dojo/SKILL.md
```

Expected: check-ignore 無輸出、exit=1；prettier 無錯誤。

---

### Task 6: 全量檢查、commit、PR

**Files:** 無新檔（git 操作）

- [ ] **Step 1: 全 repo prettier check（模擬 CI lint job）**

```bash
pnpm exec prettier --check .
```

Expected: `All matched files use Prettier code style!`（exit 0）。若失敗，只准修本計畫新增的檔案。

- [ ] **Step 2: Commit 內容（精準 add，絕不 add docs/design）**

```bash
git add docs/learning/feynman-curriculum.md docs/learning/feynman-ledger.md \
        .claude/skills/feynman-dojo/SKILL.md \
        docs/superpowers/specs/2026-07-13-feynman-dojo-design.md \
        docs/superpowers/plans/2026-07-13-feynman-dojo.md
git status --short   # 確認 staged 清單無 docs/design/*
git commit -m "docs(docs): Feynman Dojo 內化訓練系統（skill + 七項架構課綱 + 進度帳本）"
```

- [ ] **Step 3: Push 分支＋開 PR（base main）**

```bash
git push -u origin feature/feynman-dojo
gh pr create --base main --title "docs(docs): Feynman Dojo 內化訓練系統" --body "..."
```

PR body 重點：目的（把 2026-07-13 回顧提煉的架構經驗固化＋建立費曼內化循環）、三檔案角色、`.gitignore` 例外的精準性（僅放行 feynman-dojo、openspec-* 維持不追蹤）。

- [ ] **Step 4: Merge 分級判定**

內容＝純 docs ＋ 一段 gitignore 例外。依分級授權屬低風險（純 docs）→ CI 綠後 AI 可自 merge；但 `.gitignore` 動到 repo 設定，**merge 前向 owner 明示這一點**，owner 無異議即自 merge（不刪分支）。
