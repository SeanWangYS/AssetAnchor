 ...
 * export 供 UI 層日期輸入（如期間篩選自訂起訖）複用同一驗證。
 */
...
export const transactionInputSchema = z
  .object({
    ...
    market: z.enum(MARKETS, '請選擇市場'),
    asset_type: z.enum(ASSET_TYPES, '請選擇資產類型'),
    // BUY / SELL（Sprint 5）；公司行動等其餘型別留待後續 sprint。不可超賣為表單層驗證（需衍生持倉）。
    transaction_type: z.enum(['BUY', 'SELL'], '請選擇交易類型'),
    transaction_date: z.string().trim().refine(isRealDate, '日期需為 YYYY-MM-DD'),
    currency: z.enum(MVP_CURRENCIES, '請選擇幣別'),
