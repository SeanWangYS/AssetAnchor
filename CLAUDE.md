...
Feature-based：`src/features/<feature>/`（screens + `*Service.ts` Firestore CRUD + `*Store.ts` Zustand store + 純函式）、`src/core/`（`navigation/` `theme/` `ui/`）、`src/services/firebase/`。導航是 React Navigation v7：Root Stack 包 4 個 Bottom Tab（持倉 / 交易 / 帳戶 / 設定）+ Modal group；auth 狀態決定 `RootNavigator` 顯示 AuthStack 還是 MainTabs。表單目前用受控 input + `packages/shared` 的 zod schema 做 `safeParse` 驗證（RHF 待 Sprint 3 交易表單再導入）。UI 字串純繁體中文（MVP 不做 i18n）。
